...
CLOUD_FN_9 = "https://us-central1-csci-e-599-trend-detection.cloudfunctions.net/get_citation_9"
CLOUD_FN_10 = "https://us-central1-csci-e-599-trend-detection.cloudfunctions.net/get_citation_10"
...
if __name__ == "__main__":
    ...
    # check command line argument for Start PubMed ID in between allowed min and max values
    if _start_id < _MIN_PUBMED_ID or _start_id > (_MAX_PUBMED_ID-1):
        raise ValueError(f"Start PubMed ID should be between {_MIN_PUBMED_ID} and {_MAX_PUBMED_ID-1}.  {ERROR_MSG}")     
    ...
    # check command line argument for End PubMed ID in between allowed min and max values
    if _end_id < (_MIN_PUBMED_ID+1) or _end_id > _MAX_PUBMED_ID:
        raise ValueError(f"End PubMed ID should be between {_MIN_PUBMED_ID+1} and {_MAX_PUBMED_ID}")

    # check command line argument for Start PubMed ID < End PubMed ID
    if _start_id > _end_id:
        raise ValueError(f"End PubMed ID should be greater than Start PubMed ID. {ERROR_MSG}")             
    ...
    if _max_workers < 2 or _max_workers > 16:
        raise ValueError(f"Number of parallel workers should be between 2 and 16. {ERROR_MSG}")         
    ...
    print(f"Processed {_end_id-_start_id+1} PubMed Articles in {round(tot_time.total_seconds()/60,2)} minutes")
